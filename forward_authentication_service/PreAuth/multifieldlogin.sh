...
write_log () {
	...
	if [ ! -z $logname ]; then
		...
		if [ $sizeratio -ge $min_freespace_to_log_ratio ]; then
			userinfo="username=$username, emailAddress=$emailaddr, phone=$phone, addr=$addr, code=$code"
...
if [ ! -z "$status_present" ]; then
	...
	hid="0"
	gatewayaddress="0"
	queryvarlist="clientip gatewayname redir username phone emailaddr addr code"
else
	queryvarlist="clientip gatewayname hid gatewayaddress redir username phone emailaddr addr code"
fi
...
phone=$(printf "${phone//%/\\x}")

emailaddr=$(printf "${emailaddr//%/\\x}")

addr=$(printf "${addr//%/\\x}")
htmlentityencode "$addr"
addrhtml=$entityencoded

code=$(printf "${code//%/\\x}")
htmlentityencode "$code"
codehtml=$entityencoded
...
login_form="
	...
	<input type=\"tel\" name=\"phone\" value=\"$phone\" autocomplete=\"on\" ><br>Phone<br><br>
	<input type=\"email\" name=\"emailaddr\" value=\"$emailaddr\" autocomplete=\"on\" ><br>Email<br><br>
	<input type=\"text\" name=\"addr\" value=\"$addrhtml\" autocomplete=\"on\" ><br>Address<br><br>
	<input type=\"text\" name=\"code\" value=\"$codehtml\" ><br>Enter Access Code<br><br>
...
#
if [ -z "$username" ] || [ -z "$phone" ] || [ -z "$emailaddr" ] || [ -z "$addr" ] || [ -z "$code" ]; then
	...
	# eg:
	requested="http://onboard-wifi.net"
