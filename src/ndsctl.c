...
usage(void)
{
	printf(
		...
		"  auth mac|ip|token sessiontimeout(minutes, optional) upload(optional, reserved) download(optional, reserved)\n"
		"	Authenticate client with specified mac, ip or token\n"
		"	sessiontimeout sets the session duration. Unlimited if 0, defaults to global setting if not specified.\n"
		"	upload and download are reserved for future use.\n\n"
		"  deauth mac|ip|token\n"
