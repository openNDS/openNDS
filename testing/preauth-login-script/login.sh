...
write_log () {
	...
	if [ $sizeratio -ge $min_freespace_to_log_ratio ]; then
		userinfo="username=$username, phone=$phone, emailAddress=$emailaddr, address=$address"
...
if [ ! -z "$status_present" ]; then
	...
	hid="0"
	gatewayaddress="0"
	queryvarlist="clientip gatewayname redir username phone emailaddr address"
else
	queryvarlist="clientip gatewayname hid gatewayaddress redir username phone emailaddr address"
fi
...
htmlentityencode "$phone"
phonehtml=$entityencoded

address=$(printf "${address//%/\x}")
htmlentityencode "$address"
addresshtml=$entityencoded
...
login_form="
	...
	<input type=\"text\" name=\"phone\" value=\"$phonehtml\" autocomplete=\"on\" ><br>Phone<br><br>
	<input type=\"email\" name=\"emailaddr\" value=\"$emailaddr\" autocomplete=\"on\" ><br>Email<br><br>
	<input type=\"text\" name=\"address\" value=\"$addresshtml\" autocomplete=\"on\" ><br>Address<br><br>
...
#
if [ -z "$username" ] || [ -z "$emailaddr" ] || [ -z "$phone" ] || [ -z "$address" ]; then
